...
rs_image=os.path.join(folder,'rs_image')
rs_pcd=os.path.join(folder,'rs_pcd')

for p in [image0,image1,image2,image3,pcd,rs_image,rs_pcd]:
    ...
    def __init__(self):
        ...
        with rosbag.Bag(bag_path, 'r') as bag:  # 要读取的bag文件；
            for topic, msg, t in bag.read_messages():
                ...
                # elif topic == "/camera/depth/color/points":  # laser的topic；
                #     timestr = "%.6f" % msg.header.stamp.to_sec()
                #     laser_data_name = timestr + ".txt"
                #     laser_data_path = os.path.join(rs_pcd, laser_data_name)
                #     lidar = pc2.read_points(msg)
                #     points = np.array(list(lidar))
                #     points=points[:,:3]
                #     dist=np.sqrt(np.sum(np.square(points),axis=1))
                #     points=points[np.where(dist<4)]
                #     np.savetxt(laser_data_path,points)
                #     print laser_data_path
                elif topic == "/camera/color/image_raw":  # 图像的topic；
                    try:
                        cv_image = self.bridge.imgmsg_to_cv2(msg, "bgr8")
                    except CvBridgeError as e:
                        print e
                    ...
                    # %.6f表示小数点后带有6位，可根据精确度需要修改；
                    image_name = timestr + ".png"  # 图像命名：时间戳.jpg
                    image_name=os.path.join(rs_image, image_name)
                    cv2.imwrite(image_name, cv_image)  # 保存；
                    print image_name
